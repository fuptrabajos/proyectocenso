    ...
    const onSubmit = handleSubmit(async data => {
        try {
            if(params.id) {
                ...
                toast.success('Tipo de régimen actualizado con éxito', {
                ...
                toast.success('Nuevo tipo de régimen creado con éxito', {
                    position: "bottom-right",
                    style: {
                        background: "#047857", // Verde oscuro
                        color: "#fff",
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-6 max-w-xl mx-auto">
            <h1 className="text-2xl font-bold text-gray-800 mb-6">
                {params.id ? 'Editar Tipo de Régimen' : 'Crear Nuevo Tipo de Régimen'}
            </h1>
            
            <form onSubmit={onSubmit} className="space-y-4">
                <div>
                    <label htmlFor="des_regimen" className="block text-gray-700 font-medium mb-1">Tipo de Régimen</label>
                    <input
                        type="text"
                        id="des_regimen"
                        placeholder="Ingrese el tipo de régimen"
                        {...register("des_regimen", { required: "Este campo es requerido" })} 
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    />
                    {errors.des_regimen && (
                        <p className="text-red-500 text-sm mt-1">{errors.des_regimen.message}</p>
                    )}
                </div>
                        ...
                        onClick={async () => {
                            ...
                            if (accepted) {
                                try {
                                    ...
                                    toast.success('Tipo de régimen eliminado correctamente', {
                                        position: "bottom-right",
                                        style: {
                                            background: "#047857",
                                            color: "#fff",
            ...
            <div className="mt-4">
                <button
                    ...
                    onClick={() => navigate('/dashboard/Regimen')}
                >
                    <svg className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
